...
const AddFlashcard = (props) => {
    ...
    async function addCollection() {
        ...
        await axios({
          method: "post",
          url: `http://127.0.0.1:8000/api/flashcard/addflashcard/${props.collectionId}/`,
          headers: {
            Authorization: "Bearer " + jwt
          },
          data: {"collection": props.collectionId, ...formValues}
        }).then(response => {
            window.location = "/collection";
        }
        ).catch(error => {
          alert("Flashcard not able to be added at this time. Please try again later.")
        })
      }
